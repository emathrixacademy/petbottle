...
#define SWING_LIMIT_MS   30000UL  // ~180° travel at speed 5; recalibrate if needed
unsigned long swingLimitTimer = 0;
int swingCurrentDir = 0;  // -1=left, 0=stopped, +1=right
...
void executeCmd(String cmd) {
  ...
  if (cmd.startsWith("PI") || cmd.startsWith("pi")) {
    ...
    if (pi.startsWith("FW")) {
      ...
      swingSpeed = 5;
      if (swingCurrentDir != -1) { swingLimitTimer = millis(); swingCurrentDir = -1; }
      setSwing(-1);
  ...
  if (currentMode == MODE_SWING) {
    switch (c0) {
      ...
      case 'S': stopSwing(); swingCurrentDir = 0; Serial.println("STOPPED"); break;
      case '+':
...
void loop() {
  ...
  // Swing 180° hard limit — auto-stop when timer expires
  if (swingCurrentDir != 0 && millis() - swingLimitTimer >= SWING_LIMIT_MS) {
    stopSwing();
    swingCurrentDir = 0;
