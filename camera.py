#!/usr/bin/env python3
"""
Test best.hef detection using Raspberry Pi Camera
- Live camera feed with detection overlay
- Press 'S' to save a snapshot
- Press 'Q' to quit
- Results saved to ./snapshots/
"""

import cv2
import numpy as np
import time
import os
from pathlib import Path
from picamera2 import Picamera2
from hailo_platform import (
    HEF, VDevice, HailoStreamInterface,
    InferVStreams, ConfigureParams,
    InputVStreamParams, OutputVStreamParams, FormatType
)

# ── Settings ───────────────────────────────────────────────
HEF_PATH       = "best.hef"
CLASS_NAMES    = ["PET-Bottle"]
CONF_THRESHOLD = 0.3
INPUT_SIZE     = (416, 416)
CAMERA_SIZE    = (1280, 720)
CAMERA_FPS     = 30
REG_MAX        = 16
SNAPSHOT_DIR   = "./snapshots"
# ───────────────────────────────────────────────────────────


def generate_anchors(input_size=416, strides=[8, 16, 32]):
    anchors = []
    for stride in strides:
        grid_size = input_size // stride
        for y in range(grid_size):
            for x in range(grid_size):
                anchors.append((x + 0.5, y + 0.5, stride))
    return anchors

ANCHORS = generate_anchors()


def dfl_decode(reg, reg_max=16):
    N = reg.shape[0]
    reg = reg.reshape(N, 4, reg_max)
    reg = reg - reg.max(axis=-1, keepdims=True)
    exp = np.exp(reg)
    reg = exp / exp.sum(axis=-1, keepdims=True)
    weights = np.arange(reg_max, dtype=np.float32)
    return (reg * weights).sum(axis=-1)


def postprocess(outputs, orig_w, orig_h):
    conf_raw = outputs.get("best/activation1")
    reg_raw  = outputs.get("best/concat14")
    if conf_raw is None or reg_raw is None:
        return []

    conf = conf_raw.reshape(-1)
    reg  = reg_raw.reshape(-1, 64)
    ltrb = dfl_decode(reg, REG_MAX)

    detections = []
    for i, (score, (l, t, r, b)) in enumerate(zip(conf, ltrb)):
        if score < CONF_THRESHOLD:
            continue
        cx, cy, stride = ANCHORS[i]
        x1 = int((cx - l) * stride * orig_w / INPUT_SIZE[0])
        y1 = int((cy - t) * stride * orig_h / INPUT_SIZE[1])
        x2 = int((cx + r) * stride * orig_w / INPUT_SIZE[0])
        y2 = int((cy + b) * stride * orig_h / INPUT_SIZE[1])
        x1, y1 = max(0, x1), max(0, y1)
        x2, y2 = min(orig_w, x2), min(orig_h, y2)
        if x2 <= x1 or y2 <= y1:
            continue
        detections.append([x1, y1, x2, y2, float(score), 0])

    if not detections:
        return []

    boxes  = [[d[0], d[1], d[2]-d[0], d[3]-d[1]] for d in detections]
    scores = [d[4] for d in detections]
    indices = cv2.dnn.NMSBoxes(boxes, scores, CONF_THRESHOLD, 0.45)
    if len(indices) == 0:
        return []
    return [detections[i] for i in indices.flatten()]


def draw_detections(frame, detections):
    for (x1, y1, x2, y2, conf, cls_id) in detections:
        label = CLASS_NAMES[int(cls_id)] if int(cls_id) < len(CLASS_NAMES) else f"cls{cls_id}"
        color = (0, 255, 0)
        cv2.rectangle(frame, (x1, y1), (x2, y2), color, 4)
        text = f"{label.upper()} {conf:.2f}"
        (tw, th), _ = cv2.getTextSize(text, cv2.FONT_HERSHEY_SIMPLEX, 0.6, 2)
        cv2.rectangle(frame, (x1, y1 - 25), (x1 + tw + 10, y1), color, -1)
        cv2.putText(frame, text, (x1 + 5, y1 - 7),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 0), 2)
    return frame


def draw_hud(frame, fps, detections, snapshot_count):
    """Draw HUD overlay with stats"""
    h, w = frame.shape[:2]

    # FPS
    cv2.putText(frame, f"FPS: {fps:.1f}", (20, 40),
                cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)

    # Detection count
    color = (0, 255, 0) if len(detections) > 0 else (0, 200, 255)
    cv2.putText(frame, f"Detections: {len(detections)}", (20, 80),
                cv2.FONT_HERSHEY_SIMPLEX, 0.8, color, 2)

    # Hailo label
    cv2.putText(frame, "Hailo AI Hat", (20, 115),
                cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 200, 0), 2)

    # Snapshot count
    cv2.putText(frame, f"Snapshots: {snapshot_count}", (20, 148),
                cv2.FONT_HERSHEY_SIMPLEX, 0.6, (200, 200, 200), 2)

    # Controls hint at bottom
    cv2.putText(frame, "S=Snapshot  Q=Quit", (20, h - 20),
                cv2.FONT_HERSHEY_SIMPLEX, 0.6, (180, 180, 180), 1)

    return frame


def main():
    Path(SNAPSHOT_DIR).mkdir(parents=True, exist_ok=True)

    print(f"Loading HEF: {HEF_PATH}")
    hef = HEF(HEF_PATH)

    with VDevice() as target:
        configure_params = ConfigureParams.create_from_hef(
            hef, interface=HailoStreamInterface.PCIe
        )
        network_groups = target.configure(hef, configure_params)
        network_group = network_groups[0]
        network_group_params = network_group.create_params()

        input_vstreams_params = InputVStreamParams.make(
            network_group, format_type=FormatType.FLOAT32
        )
        output_vstreams_params = OutputVStreamParams.make(
            network_group, format_type=FormatType.FLOAT32
        )
        input_name = hef.get_input_vstream_infos()[0].name

        # Setup camera
        print("Starting camera...")
        picam2 = Picamera2()
        config = picam2.create_preview_configuration(
            main={"size": CAMERA_SIZE, "format": "BGR888"},
            controls={"FrameRate": CAMERA_FPS}
        )
        picam2.configure(config)
        try:
            picam2.set_controls({"HdrMode": 0, "AfMode": 2})
        except Exception as e:
            print(f"Camera warning: {e}")
        picam2.start()
        print("✅ Camera started")

        window_name = "HEF Detection Test - S=Snapshot Q=Quit"
        cv2.namedWindow(window_name, cv2.WINDOW_NORMAL)

        snapshot_count = 0
        frame_count = 0
        fps = 0
        fps_timer = time.time()

        with InferVStreams(network_group, input_vstreams_params, output_vstreams_params) as infer_pipeline:
            with network_group.activate(network_group_params):
                print("✅ Running! Press S to save snapshot, Q to quit.")
                print(f"   Snapshots saved to: {SNAPSHOT_DIR}/")

                try:
                    while True:
                        start = time.time()

                        frame = picam2.capture_array()
                        if frame is None:
                            continue

                        orig_h, orig_w = frame.shape[:2]

                        # Preprocess
                        img = cv2.resize(frame, INPUT_SIZE)
                        img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
                        img = img.astype(np.float32) / 255.0
                        input_data = np.expand_dims(img, axis=0)

                        # Infer
                        raw_outputs = infer_pipeline.infer({input_name: input_data})
                        infer_ms = (time.time() - start) * 1000

                        # Postprocess
                        detections = postprocess(raw_outputs, orig_w, orig_h)

                        # Draw
                        display = draw_detections(frame.copy(), detections)
                        display = draw_hud(display, fps, detections, snapshot_count)

                        # Inference time
                        cv2.putText(display, f"Infer: {infer_ms:.1f}ms", (orig_w - 200, 40),
                                    cv2.FONT_HERSHEY_SIMPLEX, 0.6, (200, 200, 200), 1)

                        cv2.imshow(window_name, display)

                        # FPS calc every 30 frames
                        frame_count += 1
                        if frame_count % 30 == 0:
                            fps = 30 / (time.time() - fps_timer)
                            fps_timer = time.time()

                        # Key handling
                        key = cv2.waitKey(1) & 0xFF
                        if key == ord('q') or key == ord('Q'):
                            break
                        elif key == ord('s') or key == ord('S'):
                            snapshot_count += 1
                            ts = time.strftime("%Y%m%d_%H%M%S")
                            snap_path = f"{SNAPSHOT_DIR}/snap_{ts}_{len(detections)}det.jpg"
                            cv2.imwrite(snap_path, display)
                            print(f"📸 Snapshot saved: {snap_path} ({len(detections)} detections)")

                except KeyboardInterrupt:
                    print("\nShutting down...")
                finally:
                    try:
                        picam2.stop()
                        picam2.close()
                    except:
                        pass
                    cv2.destroyAllWindows()

    print(f"\n✅ Done. {snapshot_count} snapshots saved to {SNAPSHOT_DIR}/")


if __name__ == "__main__":
    main()